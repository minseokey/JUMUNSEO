...
public interface OpinionJPARepository extends JpaRepository<Opinion, Long> {
    ...
    @Query("SELECT new com.jumunseo.debate.domain.opinion.dto.OpinionResponseDto(o.content,o.type,o.side,o.userEmail,o.id) FROM Opinion o WHERE o.subject.id = :subjectId AND o.id BETWEEN :last AND :last + 10 ORDER BY o.id DESC")
    List<OpinionResponseDto> find10BySubjectIdWithLast(Long subjectId, Long last);
    @Query("SELECT new com.jumunseo.debate.domain.opinion.dto.OpinionResponseDto(o.content,o.type,o.side,o.userEmail,o.id) FROM Opinion o WHERE o.subject.id = :subjectId ORDER BY o.id DESC LIMIT 10")
